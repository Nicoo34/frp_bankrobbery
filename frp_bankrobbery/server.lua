...
local data = {
    [2] = {
        staticName = "Banque de Saint Denis",
        staticReward = 50000,
...
AddEventHandler(
    ...
    function(index, participants)
        ...
        if interiorIndexBeingRobbed ~= nil then
            ...
            TriggerClientEvent("FRP:NOTIFY:Simple", _source, "Cet endroit est déjà cambriolé")
            return
        end
        ...
        if cooldownEndsAtTimeStamp > os.time() then
            ...
            TriggerClientEvent("FRP:NOTIFY:Simple", _source, "On s'est fait cambrioler il y a peu de temps, on n'a pas d'argent")
            return
        else
        ...
        if 6 < 5 then
            TriggerClientEvent("FRP:NOTIFY:Simple", _source, "Cette banque ne peut pas être volée, pas assez de police.")
            return
        end
            ...
            if numParticipantsToCheck > 0 then
                for _, participantSource in pairs(participants) do
                    if playerSource == participantSource then
                        ...
                        TriggerClientEvent("FRP:ROBBERY:InitVaultRobberyStart")
        ...
        API.NotifyUsersWithGroup("trooper", "Un braquage de banque a commencé " .. data[index].staticName)
    end
)
...
function endRobberyGiveReward()
    ...
    for participantSource, _ in pairs(indexBeingRobbed_participants) do
        ...
        TriggerEvent("FRP:COMBATLOG:AddUserCombatReason", user_id, 300, "braquage de banque")
    ...
    TriggerClientEvent("FRP:TOAST:New", -1, "alert", "le braquage est terminé!")
